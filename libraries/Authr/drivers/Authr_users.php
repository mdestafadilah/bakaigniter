 ...
 * @category    Security
 */
class Authr_users extends CI_Driver
{
    /**
    ...
    {
        log_message('debug', "#Authr: Users Driver Initialized");
    }
    ...
    public function get( $term, $key = 'id' )
    {
        ...
        {
            log_message('error', '#Authr: Users->get failed identifying user using '.$key.' field.');
            return FALSE;
        }
    ...
    public function change_status( $user_id, $new_status, $invert = FALSE, $extra = array() )
    {
        ...
        {
            log_message('error', '#Authr: Users->change_status failed to change status.');
            return FALSE;
        }
    ...
    public function activate( $user_id, $activation_key )
    {
        ...
        if ( $query->num_rows() == 0 )
        {
            log_message( 'error', '#Authr: Users->activate Can\'t find inactive user with ID='.$user_id.'.');
            return FALSE;
        }
